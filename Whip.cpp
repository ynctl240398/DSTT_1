	...
	if (state == WHIP_STATE_ATTACK)
	{
		...
		if (direction == -1)
		{
			...
			else
			{
				...
				this->pos = { position.x - 53, position.y + 14};
				//if (animations[ani]->getDone())
				//{
				//	quarter = Behind;
				//	animations[ani]->setDone(false);
		...
		else
		{
			if (quarter == Behind)
			{
				...
				//	quarter = Front;
				//	animations[ani]->setDone(false);
				//	animations[ani]->setCurrentFrame(0);
				//}
			}
			else
			{
			...
			//	this->pos = { position.x + 32, position.y };
			//}
			//else if (animations[ani]->GetCurrentFrame() < animations[ani]->GetTotalFrame() - 1) 
			//	this->pos = { position.x - 29, position.y };
		}
...
void CWhip::CreateAnimation()
{
	...
	sprites->Add(20003, 215, 28, 268, 47, texWhip); //stand attack left 1 truoc
	...
	ani = new CAnimation(100);  //stand attack right 1
	ani->Add(20010);
	ani->Add(20011);
	ani->Add(20012);
	//ani->Add(20013);
